...
func confirmTx(ctx context.Context, require *require.Assertions, uri string, txID ids.ID, receiverAddr codec.Address, receiverExpectedBalance uint64) {
	...
	require.Equal(consts.TransferTokenID, transferOutputBytes[0])
	...
	require.NoError(err)
	// transferOutput, ok := transferOutputTyped.(*actions.TransferTokenResult)
	// require.True(ok)
	// require.Equal(receiverExpectedBalance, transferOutput.TransferTokenID)
}
